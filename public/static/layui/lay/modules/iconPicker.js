layui.define(['laypage', 'form'], function (exports) {
    ...
    IconPicker.prototype.render = function(options){
        var opts = options,
            ...
            // 是否使用标题
            title=(opts.title==(null||false))? '1':opts.title==true ? TIPS:opts.title,
            // 是否分页：true/false
            page = opts.page,
            // 每页显示数量
        ...
        var a = {
            ...
            createBody: function () {
                // 获取数据
                var searchHtml =  '',titleHtml='';

                if(title!=''){
                    titleHtml='<div class="popover-title">'+title+'</div>';
                }
                ...
                var bodyHtml = '<div class="layui-iconpicker-body" id="'+ PICKER_BODY +'">' +
                    searchHtml +
                     ...
                     '</div>';
                $('#' + ICON_BODY).find('.layui-anim').eq(0).html(titleHtml+bodyHtml);
